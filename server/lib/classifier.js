...
function getClassification(emailBody) {
  ...
  classifier.train();
  const repliedEmail = extractReply(emailBody);
  console.log("replied Email", repliedEmail);
  return classifier.classify(extractKeywords(repliedEmail));
}
...
function extractReply(emailBody){

return planer.extractFromPlain(emailBody);
}
